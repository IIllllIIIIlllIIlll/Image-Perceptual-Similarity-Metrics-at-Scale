...
from hashing.SimCLRv1 import resnet_wider as SIMv1  # NEED TO BE DOWNLOADED
from hashing.SimCLRv2 import resnet as SIMv2  # NEED TO BE DOWNLOADED
...
def cosine_distance(vector, database):
    ...
    dot_product = (vector*database).sum(dim=1)  # why not vector.dot(database)?
...
class NeuralAlgorithm(Algorithm):
   ...
   Attributes
    ...
    def __init__(self, algorithm, hash_size=8, raw_features=True, distance='cosine',
        ...
        self.device = device
        # self.model is not defined. Everything that is used within the self.<> nest as a
        # constant needs to be defined here
