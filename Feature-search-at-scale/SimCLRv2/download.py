...
Created on Mon Jul 11 16:21:47 2022
 ...
 Code Reviewer: a script to download the weights used by SimCLRv2
...
mapping = {'finetune_100pct': 'Finetuned_100pct', 'finetune_10pct': 'Finetuned_10pct',
           'pretrain': 'Pretrained', 'pretrained': 'Pretrained'}
...
def save_response_content(response, destination):
    ...
    with open(destination, 'wb') as f:
        # Code Reviewer: you can also do a 'wa' and just keep appending the data as it comes
        for data in tqdm(response.iter_content(chunk_size=chunk_size),
                         leave=False, total=total):
            f.write(data)
...
if __name__ == '__main__':
    ...
    output_path = 'torch_checkpoints/' + mapping[category] + '/' 
