...
def cluster_representation(images):
    ...
    Code Reviewer: looks like it's **all** images from the same cluster?
...
ALLOWED_PARTITIONS = [
    'full_dataset',
    'clean_dataset',
    ]  # Code Reviewer: once again, that's a good candidate to move to a configs file
...
# TODO: this hardpatch is required to load 4chan images for analysis - uncomment it
# def get_features(partition, algorithm):
#     """
#     Extract the features and mapping corresponding to the wanted partition of
#     the kaggle memes dataset.
...
#
#
#     features, mapping = utils.load_features(algorithm, 'chan_images', to_bytes=False)
...
def extract_params_from_folder_name(directory):
    ...
    """
    # Pathlib is your friend once again
    if directory[-1] == '/':
        directory = directory.rsplit('/', 1)[0]
